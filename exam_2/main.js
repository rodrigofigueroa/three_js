...
function lineTiempo(){
    ...
    $('.linea-b').on('click', '.meses-linea', function() {
        ...
        var newaumTimeLine = aumTimeLine * numMes;
    ...
    function initDos(){
			...
			scene.add( direcctional );
				...
				function onDocumentMouseMove( event ) {
					...
					mouse.y = ( event.clientX / container.offsetHeight ) * 2 - 1;
                    var mousy = (event.clientX/container.offsetWidth) * 20;                    

                    if(mousy > 13 && camera.position.x < 18.6){
                        avanza(mousy);
                        console.log('avanzando');					
					}else if(mousy < 10 && camera.position.x > 4.8){
                        retrocede(mousy);
                        console.log('retrocede');
                    }	
                    if(camera.position.x >= 16.6){
                    ...
                    console.log(camera.position.x);
                        // if(mousy <= 5.9 ){
                        //     tl.to(camera.position, 3, {x: 4.8, z: 2, y: 2.3});	
                        ...
                        // if(mousy > 6 && mousy < 12){
                        //     tl.to(camera.position, 3, {x: 15,z: -1, y: 1.5});
				...
				function retrocede(mousy){
                    var mousy = mousy;
                    camera.position.x += -0.07;
                    console.log('sdas',camera.position.z);
